...
func TestRead(t *testing.T) {
	...
	for _, contract := range contList {
		form = url.Values{
			"Value": {fmt.Sprintf(contract, name)}, "ApplicationId": {`1`},
			"Conditions": {`true`},
		}
	...
	assert.NoError(t, postTx(`EditColumn`, &url.Values{
		`TableName`: {name}, `Name`: {`active`},
	...
	form = url.Values{
		"Name": {name}, "InsertPerm": {`ContractConditions("MainCondition")`},
